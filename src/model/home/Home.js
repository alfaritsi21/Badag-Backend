...
module.exports = {
  get_worker_model: (limit, offset, sort, search) => {
    return new Promise((resolve, reject) => {
      connection.query(
        `SELECT users.user_name, users.user_id, users.user_email, users.user_job, users.user_location, users.user_phone, users.user_time_job,  users.user_work_location,  users.user_description, GROUP_CONCAT(skills.skill) AS skills, users.user_image FROM users RIGHT JOIN skills on skills.id_user = users.user_id WHERE skills.skill LIKE '%${search}%' GROUP BY skills.id_user ORDER BY ${sort} LIMIT ? OFFSET ?`,
        [limit, offset],
        (error, result) => {
