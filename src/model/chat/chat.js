...
module.exports = {
  get_room_chat_company: (id_recruter) => {
    return new Promise((resolve, reject) => {
      connection.query(
        "SELECT * FROM roomchat LEFT JOIN users ON users.user_id = roomchat.user_id WHERE roomchat.company_id = ?",
        id_recruter,
        (error, result) => {
          console.log(result);
  ...
  get_room_chat_worker: (id_worker) => {
    return new Promise((resolve, reject) => {
      connection.query(
        "SELECT * FROM roomchat LEFT JOIN company ON company.company_id = roomchat.company_id WHERE roomchat.user_id = ?",
        id_worker,
        (error, result) => {
          !error ? resolve(result) : reject(new Error(error));
        }
