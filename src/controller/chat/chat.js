...
const { get_room_chat_company, get_room_chat_worker, get_message, post_message_model } = require('../../model/chat/chat')
...
module.exports = {
  get_chat_room_company: async (request, response) => {
    ...
    try {
      ...
      client.setex(`get_chatroom_company,company_id:${company_id}`, 3600, JSON.stringify(result))
  ...
  get_chat_room_worker: async (request, response) => {
    ...
    try {
      ...
      client.setex(`get_chatroom_worker,worker_id:${user_id}`, 3600, JSON.stringify(result))
      return helper.response(response, 200, "Get Chat Success", result)
    } catch (error) {
      return helper.response(response, 400, "Bad Request", error)
    }
  ...
  get_message: async (request, response) => {
    ...
    try {
      ...
      client.setex(`get_message,roomchat_id:${roomchat_id}`, 3600, JSON.stringify(result))
  ...
  post_message: async (request, response) => {
    let { roomchat_id, sender, receive, message } = request.body // company / worker login
    try {
      const setData = {
        roomchat_id: roomchat_id,
        sender: sender,
        receive: receive, // Dari frontend ngirim id_penerima siapa
        message: message,
        created_at: new Date()
      }
